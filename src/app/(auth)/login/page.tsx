...
export default function LoginPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    login.mutate(form, {
      onSuccess: (json) => {
        const { onboardingComplete } = json.data.user;
        router.replace(onboardingComplete ? "/home" : "/onboarding");
      },
      onError: (err) => {
        if (err instanceof ApiError && err.status === 403) return;
        const message =
          err instanceof ApiError ? err.message : "Couldn’t log in. Try again.";
        toast.error(message);
      },
    });
  };
  ...
  const goVerify = () => {
    ...
    resend.mutate(form.identifier, {
      onSettled: () =>
        router.push(`/verify?email=${encodeURIComponent(form.identifier)}`),
    });
  };
