...
export default function HomePage() {
  ...
  const actions = [
    { label: 'Discover', sub: 'Find your twins', href: '/discover', icon: Compass, tint: 'var(--blob-powder)' },
    { label: 'Circles', sub: 'Your communities', href: '/communities', icon: Sparkles, tint: 'var(--blob-lavender)' },
    { label: 'Messages', sub: chatUnread.data?.data.total ? `${chatUnread.data.data.total} unread` : 'Your chats', href: '/chat', icon: Users, tint: 'var(--blob-sage)' },
    { label: 'Search', sub: 'Find anyone', href: '/search', icon: Search, tint: 'var(--blob-peach)' },
  ];
    ...
    <motion.div variants={stagger} initial="hidden" animate="show"
      ...
      <div className="flex flex-col gap-5">
        <motion.header variants={fadeUp}
          className="relative overflow-hidden rounded-[28px] bg-charcoal p-7 text-white">
          <motion.div
          ...
          <div className="relative">
            ...
            {alerts.data?.data.count ? (
              <Link href="/notifications"
                className="mt-5 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-[13px] font-medium backdrop-blur transition-colors hover:bg-white/15">
        ...
        <div className="grid grid-cols-3 gap-2">
          {stats.map((s) => (
            <Link key={s.label} href={s.href}
              className="card flex flex-col items-center gap-1 py-4 transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-float)]">
              <span className={`font-display text-[28px] font-semibold ${s.accent && s.value > 0 ? 'text-celebrate' : ''}`}>
          ...
          {actions.map((a) => (
            <motion.div key={a.label} whileHover={{ y: -3 }} whileTap={{ scale: 0.97 }}
              ...
              <Link href={a.href}
                ...
                <div className="absolute -right-3 -top-3 size-14 rounded-full opacity-70 transition-transform duration-500 group-hover:scale-150"
                  style={{ background: a.tint }} />
                <a.icon size={20} className="relative text-ink" />
                <div className="relative">
