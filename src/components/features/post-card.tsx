...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <article
      ...
      style={
        post.isBirthdayPost
          ? { boxShadow: "0 0 0 2px var(--celebrate), var(--shadow-card)" }
          : undefined
      }
    >
      {post.isBirthdayPost && (
        <div
          className="relative flex items-center gap-2 overflow-hidden px-5 py-2.5 text-[13px] font-semibold text-[#8a6410]"
          style={{
            background:
              "linear-gradient(90deg, var(--celebrate-soft), transparent)",
          }}
        >
          <motion.span
      ...
      {showReactions && (
        <div className="px-4 pb-2">
          <ReactionsDetail postId={post.id} />
        </div>
      )}
